...
export function FigmaShopCard({ category }: Props) {
  ...
  const message = encodeURIComponent(
    `Hi, I want details for ${category.breed} (${category.lifeStage}). Source=home_card slug=${category.slug}`,
  );
  const contacts = [
    {
    ...
    {
      href: `mailto:${site.email}?subject=${encodeURIComponent(`Inquiry: ${category.breed} [home_card ${category.slug}]`)}`,
      label: "Email",
